...
const ProfileCard = ({ name, title, handle, avatarUrl, className = "" }) => {
  return (
    <div className={`group relative h-[400px] md:h-[450px] w-full max-w-[320px] overflow-hidden bg-zinc-900 rounded-xl border border-zinc-800 transition-all duration-300 hover:border-[#FFA200]/50 hover:shadow-2xl hover:shadow-[#FFA200]/10 ${className}`}>
      ...
      {/* 2/3 Image Section - strictly 66.66% */}
      <div className="relative h-[66.66%] w-full overflow-hidden">
      ...
      {/* 1/3 Content Section - strictly 33.33% */}
      <div className="relative h-[33.33%] w-full bg-zinc-900 px-6 pb-4 pt-4 flex flex-col justify-between z-20">
        ...
        <div className="w-full">
          <h3 className="font-zentry text-xl md:text-2xl font-bold uppercase text-white leading-tight mb-1 group-hover:translate-x-1 transition-transform duration-300 truncate">
            {name}
          </h3>
          <p className="font-sans text-[10px] text-[#FFA200] tracking-widest uppercase truncate">
                / {title} /
          </p>
        </div>
        ...
        <div className="flex items-end justify-between w-full">
          ...
          <a
            ...
            className="flex items-center justify-center w-10 h-10 rounded-full bg-zinc-800 text-zinc-400 border border-zinc-700 hover:border-[#FFA200] hover:bg-[#FFA200] hover:text-black transition-all duration-300 group-hover:scale-110 shrink-0"
            aria-label="LinkedIn"
          >
